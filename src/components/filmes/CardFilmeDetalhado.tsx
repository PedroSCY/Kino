  ...
  return (
    <Container>
      ...
      >
        ...
        <Flex col className="m-3 md:ml-8 gap-4 lg:gap-8 text-xl items-start">
          <Titulo
            texto={filme.titulo}
            alinhar="center"
            className="lg:text-start m-0 lg:my-5"
          />
          <Descricao texto={filme.descricao} className="text-base mt-0 lg:mt-2" />
          <p>
            Lançamento:{" "}
            {new Intl.DateTimeFormat("pt-BR").format(
              new Date(`${filme.dataDeLancamento}`),
            )}
          </p>
          <p>Duração: {filme.duracao} min</p>
          <p>Titulo Original: {filme.tituloOriginal} min</p>
          <Flex col className="justify-start items-start w-full">
            <Generos idFilme={filme.id} generosExistentes={filme.generos} grande/>
